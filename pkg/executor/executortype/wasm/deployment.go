...
func (wasm *Wasm) createOrGetDeployment(ctx context.Context, fn *fv1.Function, deployName string, deployLabels map[string]string, deployAnnotations map[string]string, deployNamespace string) (*appsv1.Deployment, error) {
	...
	if k8s_err.IsNotFound(err) {
		...
		if err != nil {
			if k8s_err.IsAlreadyExists(err) {
				depl, err = wasm.kubernetesClient.AppsV1().Deployments(deployNamespace).Get(ctx, deployName, metav1.GetOptions{})
			}
			if err != nil {
	...
	if existingDepl.Annotations[fv1.EXECUTOR_INSTANCEID_LABEL] != wasm.instanceID {
		...
		existingDepl, err = wasm.kubernetesClient.AppsV1().Deployments(deployNamespace).Update(ctx, existingDepl, metav1.UpdateOptions{})
		if err != nil {
...
func (wasm *Wasm) updateDeployment(ctx context.Context, deployment *appsv1.Deployment, ns string) error {
	_, err := wasm.kubernetesClient.AppsV1().Deployments(ns).Update(ctx, deployment, metav1.UpdateOptions{})
	return err
}

func (wasm *Wasm) deleteDeployment(ctx context.Context, ns string, name string) error {
	...
	return wasm.kubernetesClient.AppsV1().Deployments(ns).Delete(ctx, name, metav1.DeleteOptions{
		PropagationPolicy: &deletePropagation,
	})
}

func (wasm *Wasm) waitForDeploy(ctx context.Context, depl *appsv1.Deployment, replicas int32, specializationTimeout int) (latestDepl *appsv1.Deployment, err error) {
	...
	for i := 0; i < specializationTimeout; i++ {
		latestDepl, err := wasm.kubernetesClient.AppsV1().Deployments(depl.ObjectMeta.Namespace).Get(ctx, depl.Name, metav1.GetOptions{})
		if err != nil {
			return nil, err
		}
...
func (wasm *Wasm) getDeploymentSpec(ctx context.Context, fn *fv1.Function, targetReplicas *int32,
	...
	// Wasm updates the environment variable "LastUpdateTimestamp" of deployment
...
func (wasm *Wasm) scaleDeployment(ctx context.Context, deplNS string, deplName string, replicas int32) error {
	wasm.logger.Info("scaling deployment",
		zap.String("deployment", deplName),
		zap.String("namespace", deplNS),
		zap.Int32("replicas", replicas))
	_, err := wasm.kubernetesClient.AppsV1().Deployments(deplNS).UpdateScale(ctx, deplName, &autoscalingv1.Scale{
		ObjectMeta: metav1.ObjectMeta{
			Name:      deplName,
			Namespace: deplNS,
		},
